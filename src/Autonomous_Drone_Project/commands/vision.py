...
class VisionService:
    ...
    def __init__(
        ...
        # Asyncio lock for thread-safe AirSim initialization
    ...
    # -------------------------------------------------------------------------
    # AirSim camera helpers 
    # -------------------------------------------------------------------------
    def _init_airsim_sync(self):
        """Synchronously initialize AirSim."""
        import airsim
    ...
    async def _get_airsim_client(self):
        """Safely fetch or initialize the AirSim client asynchronously."""
        if self._airsim_client is not None:
            return self._airsim_client

        # Initialize lock lazily to ensure it is bound to the correct event loop
    ...
    def _fetch_frame_sync(self, client, camera_id: str):
        """Synchronously pull the image using the provided client."""
        import airsim
    ...
    # Public async API
    # -------------------------------------------------------------------------
    def get_frame(self) -> np.ndarray:  # <-- Added async
    ...
    async def detect_objects(self, frame: np.ndarray) -> List[Dict[str, Any]]: # <-- Added async
    ...
    async def save_detected_frame(self, frame: np.ndarray, detections: List[Dict[str, Any]]) -> None: # <-- Added async
        ...
        # <-- Added await (prevents disk IO from freezing the drone)
        file_path = await asyncio.to_thread(_encode_and_write) 
        self.logger.debug("Saved detection log: %s", file_path)

    async def save_frame(self, frame: np.ndarray) -> None:  # <-- Added async
        """Non-blocking disk write for raw frames."""
        if frame.size == 0:
            return
            
        def _write():
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            file_path = self.save_dir / f"raw_{timestamp}.jpg"
            cv2.imwrite(str(file_path), frame)
            return file_path
            
        # <-- Offload standard writes to a thread so cv2 doesn't block
