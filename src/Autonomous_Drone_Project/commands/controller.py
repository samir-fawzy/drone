...
class DroneController:
    ...
    def connect(self) -> None:
        ...
        try:
            self.client =  loop.run_in_executor(self._executor, self._connect_sync)
            self.connected = True
    ...
    def disconnect(self) -> None:
        if self.client is not None and self.connected:
            loop = asyncio.get_running_loop()
            loop.run_in_executor(self._executor, self._disconnect_sync, self.client)
        self.logger.info("Drone disconnected.")

    def takeoff(self) -> None:
        if self.client is not None and self.connected:
            loop = asyncio.get_running_loop()
            loop.run_in_executor(self._executor, self._takeoff_sync, self.client)
        self.logger.info("Takeoff command sent.")

    def land(self) -> None:
        if self.client is not None and self.connected:
            loop = asyncio.get_running_loop()
            loop.run_in_executor(self._executor, self._land_sync, self.client)
